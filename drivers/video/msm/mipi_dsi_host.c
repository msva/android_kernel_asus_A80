			...
			struct dcs_cmd_req *req, int rlen)
{
    ...
    //ASUS_BSP: Louis +++
    int i;
    uint32 dsi_ctrl, ctrl, intr_ctrl;
    int video_mode;
    ...
    if (video_mode) {
        ...
        MIPI_OUTP(MIPI_DSI_BASE + 0x0000, 0x1f5);
        intr_ctrl = DSI_INTR_CMD_DMA_DONE_MASK | DSI_INTR_ERROR_MASK |
                DSI_INTR_CMD_MDP_DONE_MASK;
        MIPI_OUTP(MIPI_DSI_BASE + 0x010c, intr_ctrl); /* DSI_INTL_CTRL */
    }
...
int mipi_dsi_cmd_dma_tx(struct dsi_buf *tp)
{
    ...
    if (!wait_for_completion_timeout(&dsi_dma_comp, msecs_to_jiffies(100))) {  //100 msec timeout
        printk("[Display] DCS_TX_command timeout\n");
    }
    //Louis ---
...
//Mickey+++, add wait for video eng busy with timeout
#if 0
...
static int mipi_dsi_wait_for_video_eng_busy_timeout(int timeout_us)
{
    u32 status;
    int sleep_us = 4000;
    int ret;
    ...
    /* check for VIDEO_MODE_ENGINE_BUSY */
    ret = readl_poll_timeout((MIPI_DSI_BASE + 0x0004), /* DSI_STATUS */
                status,
                (status & 0x08),
                sleep_us,
                timeout_us);
...
void mipi_dsi_cmdlist_commit(int from_mdp)
{
	...
	if (dsi_ctrl & 0x02) {
		 ...
		 */
        //Mickey+++, wait with timeout to prevent system hang
        //mipi_dsi_wait_for_video_eng_busy();
        if (mipi_dsi_wait_for_video_eng_busy_timeout(1000*1000) != 0) {
            printk("Mcikey::mipi_dsi_cmdlist_commit timeout\n");
            dump_stack();
            goto need_lock;
        }
        //Mickey---
