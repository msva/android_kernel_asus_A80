...
int mdp4_dsi_video_pipe_commit(int cndx, int wait)
{
    ...
    //mdp4_overlay_mdp_perf_upd(vctrl->mfd, 1);//Mickey+++, we don't need update here
...
//Mickey+++, allow to access vsync file even fb is in suspend state
#if 0
	if (atomic_read(&vctrl->suspend) > 0 ||
    ...
    //Mickey+++, return current time when wait for vsync timeout
    //if (ret <= 0)
    //	return -EBUSY;
    if (ret <= 0) {
...
extern bool get_camera_stream_status(void);
#if defined(CONFIG_DEBUG_FS)
extern int Camera_skip_wait4dmap_enable;
#endif
//ASUS BSP Joy --- Camera skip wait4dmap
void mdp4_dsi_video_overlay(struct msm_fb_data_type *mfd)
{
    ...
    mdp4_overlay_mdp_perf_req(mfd,(&vctrl->vlist[vctrl->update_ndx])->plist);//Mickey+++, update correct pipe bw request
...
#if defined(CONFIG_DEBUG_FS)	
                if(get_camera_stream_status() && Camera_skip_wait4dmap_enable ==1 ){
#else
                if(get_camera_stream_status() == 1){
#endif
                    pr_debug("Camera skip wait4dmap\n"); 
                } else {
                    mdp4_dsi_video_wait4dmap(cndx);
                }
            }
