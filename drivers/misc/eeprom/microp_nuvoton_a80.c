...
//       int carkit_inited;
       ...
       struct delayed_work checkP05I2C;
};
...
struct workqueue_struct *microp_check_p05_i2c_wq = NULL;

bool g_p05_i2c_busy = false;
...
int uP_nuvoton_read_reg(int cmd, void *data){
	...
	if(cmd>=0 && cmd < ARRAY_SIZE(uP_CMD_Table)){
		if(!isCMDSupportedForRead(cmd)){ // skip read for these command
			...
			while(g_p05_i2c_busy){
				printk("p05 is using i2c\n");
				msleep(50);
				
				count++;
				if(count >= 10){
					printk("check g_p05_i2c_busy count >= 10\n");
					g_p05_i2c_busy = false;
					
					if(delayed_work_pending(&g_uP_info->checkP05I2C)){
						cancel_delayed_work(&g_uP_info->checkP05I2C); 
					}
...
static void set_P05_i2c(struct work_struct *work)
{
	g_p05_i2c_busy = false;
}

void check_P05_i2c(void)
{
	queue_delayed_work(microp_check_p05_i2c_wq, &g_uP_info->checkP05I2C, 0.5*HZ);
}
...
//	info->carkit_inited=0;
	...
	microp_check_p05_i2c_wq = create_singlethread_workqueue("microp_check_p05_i2c");
	...
	INIT_DELAYED_WORK(&info->checkP05I2C, set_P05_i2c);
	// init mutex
