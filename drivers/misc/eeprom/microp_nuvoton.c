...
struct nuvoton_microp_command uP_CMD_Table[]={
        ...
        {"p05_hwid",                            0x26,  1,  E_READ,     E_MODE_APROM,     Prj_A80},          //MICROP_P05_HWID
};
...
static int isPhoneA68(void){
    return ((g_A68_hwID < A80_EVB) && (g_A68_hwID>=A68_ER1))?1:0;
}


static int isPhoneA80(void){
    return (( g_A68_hwID >= A80_EVB && g_A68_hwID <= A80_PR))?1:0;
}
...
int isCMDSupportedForRead(int cmd){
    ...
    if(isPhoneA68()){ //a68
            if(0==(uP_CMD_Table[cmd].prj & Prj_A68))
                ret=0;        
    }
...
int uP_nuvoton_read_reg(int cmd, void *data){
    ...
    if(cmd>=0 && cmd < ARRAY_SIZE(uP_CMD_Table)){
        if(!isCMDSupportedForRead(cmd)){ // skip read for these command
                      pr_debug("%s:skip cmd mode=%d, g_curr_uP_mode=%d\r\n, prj=%x", __FUNCTION__, uP_CMD_Table[cmd].mode, g_curr_uP_mode, g_A68_hwID);
        }
        else   
...
int uP_nuvoton_write_reg(int cmd, void *data){
    ...
    if(cmd>=0 && cmd < ARRAY_SIZE(uP_CMD_Table)){
        if(!isCMDSupportedForWrite(cmd)){ // skip read for these command               
                      pr_debug("%s:skip cmd mode=%d, g_curr_uP_mode=%d, prj=%x\r\n", __FUNCTION__, uP_CMD_Table[cmd].mode, g_curr_uP_mode, g_A68_hwID);
        }
        else
