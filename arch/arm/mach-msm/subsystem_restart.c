...
//ASUS_BSP+++ BennyCheng "not doing SSR during resuming"
static bool g_pm_suspend = 0;
static DECLARE_COMPLETION(subsystem_restart_pm_complete);
#define PM_COMPLETE_TIMEOUT 10* HZ

void subsystem_restart_pm_begin(void)
{
	pr_info("pm begin\n");
	INIT_COMPLETION(subsystem_restart_pm_complete);
	g_pm_suspend = 1;
}

void subsystem_restart_pm_end(void)
{
	pr_info("pm end\n");
	g_pm_suspend = 0;
	complete(&subsystem_restart_pm_complete);
}
//ASUS_BSP--- BennyCheng "not doing SSR during resuming"
...
static void subsystem_restart_wq_func(struct work_struct *work)
{
	...
	if (g_pm_suspend) {
		timeout = wait_for_completion_timeout(&subsystem_restart_pm_complete, PM_COMPLETE_TIMEOUT);
		if (!timeout) {
			pr_info("wait for pm resume timeout (%s)\n", dev->desc->name);
		} else {
			pr_info("start to restart subsystem for %s (%d ms)\n", dev->desc->name,
				jiffies_to_msecs(PM_COMPLETE_TIMEOUT) - jiffies_to_msecs(timeout));
		}
	}
	//ASUS_BSP--- BennyCheng "not doing SSR during resuming"
...
static void __subsystem_restart_dev(struct subsys_device *dev)
{
	...
	pr_info("[SSR]Restarting %s [level=%d]!\n", desc->name, restart_level);
...
// ASUS_BSP--- Wenli "config subsystem restart"
	...
	if (RESET_SOC == real_restart_level && !strcmp(name,"external_modem"))
	{
		pr_info("RED screen\n");
		preempt_disable();
		local_irq_disable();
