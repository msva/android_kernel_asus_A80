...
int wcnss_Check_CTS_Run(void)
{
    ...
    fp = filp_open("/data/misc/wifi/has_CTS_run", O_RDONLY, 0);
    if( IS_ERR(fp) ) {
				...
				struct device_attribute *attr, char *buf)
{
    ...
    pr_info("[wcnss]: wcnss_version_show, (%s).\n", penv->wcnss_version);
...
wcnss_ctrl_probe(struct platform_device *pdev)
{
    ...
    pr_err("[wcnss]: wcnss_ctrl_probe, smd_named_open_on_edge().\n");
...
wcnss_trigger_config(struct platform_device *pdev)
{
	...
	if (WCNSS_CONFIG_UNSPECIFIED == has_48mhz_xo) {
		if (has_pronto_hw) {
            ...
            pr_info("[wcnss]: WCNSS_PRONTO_HW, has_48mhz_xo=%d.\n", has_48mhz_xo);
		} else {
			penv->wcnss_hw_type = WCNSS_RIVA_HW;
			has_48mhz_xo = pdata->has_48mhz_xo;

            pr_info("[wcnss]: WCNSS_RIVA_HW, has_48mhz_xo=%d.\n", has_48mhz_xo);
		}
	}
	...
	/* allocate resources */
	penv->mmio_res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "wcnss_mmio");
