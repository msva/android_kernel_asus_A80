...
//======================================
// for cam CTS issue, bus busy , reset myDP TX chip
#define myDP_TX_P0 (0x70 >> 1)
#define myDP_TX_P1 (0x7A >> 1)
#define myDP_TX_P2 (0x72 >> 1)

#define myDP_RX_P0  (0x7e >> 1)
#define myDP_RX_P1  (0x80 >> 1)
extern void myDP_tx_hardware_recovery(void);
//======================================
...
//ASUS_BSP+++ for reset/re-init myDP TX	
	if ( (dev->adapter.nr == APQ_8064_GSBI4_QUP_I2C_BUS_ID) 
	  && ((dev->msg->addr == myDP_TX_P0) || (dev->msg->addr == myDP_TX_P1) || (dev->msg->addr == myDP_TX_P2) || (dev->msg->addr == myDP_RX_P0) || (dev->msg->addr == myDP_RX_P1)) )
	{
		dev_warn(dev->dev, "+++ Bus still busy, reset/re-init myDP TX\n");	
        
		printk("reset/re-init myDP TX\n");
		myDP_tx_hardware_recovery();
		dev_warn(dev->dev, "--- Bus still busy, reset/re-init myDP TX\n");			
	}
//ASUS_BSP--- for reset/re-init myDP TX	
recovery_end:
