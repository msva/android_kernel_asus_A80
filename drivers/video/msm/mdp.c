...
static int mdp_probe(struct platform_device *pdev)
{
	...
	if ((pdev->id == 0) && (pdev->num_resources > 0)) {
		...
		mdp_hw_cursor_init();
#endif
        if (!(mdp_pdata->cont_splash_enabled))  //ASUS_BSP: Louis
		    mdp_clk_ctrl(0);
...
{
    if (mfd->copy_splash_buf) {
#ifdef SPLASH_SCREEN_BUFFER_FOR_1080P	//ASUS_BSP: Louis miniporting
        free_boot_logo_copy_buffer();
#else	//ASUS_BSP: Louis miniporting
        dma_free_coherent(NULL, mdp_pdata->splash_screen_size,
        mfd->copy_splash_buf,
        (dma_addr_t) mfd->copy_splash_phys);
#endif	//ASUS_BSP: Louis miniporting
