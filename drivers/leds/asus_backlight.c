...
extern void renesas_set_backlight(int, bool);
...
static int mipi_set_backlight(int value)
{
	...
	else if (value > 0 && value <= pdata->phone.min_value) {
		index = pdata->phone.min_index;
	}
	else if (value > pdata->phone.min_value && value < pdata->max_value) {
	...
	if (value < pdata->phone.min_value)
            renesas_set_backlight(index, true);
	else
	    renesas_set_backlight(index, false);
	#else
...
int pad_set_backlight(int value)
{
	...
	else if (value > 0 && value <= pdata->pad.min_value) 
		index = pdata->pad.min_index;
	else if (value > pdata->pad.min_value && value < pdata->max_value) 
