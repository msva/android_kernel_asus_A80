...
void mmc_start_bkops(struct mmc_card *card, bool from_exception)
{
	...
	pr_info("%s: %s: card->ext_csd.raw_bkops_status = 0x%x, %s\n",
		mmc_hostname(card->host), __func__,
		card->ext_csd.raw_bkops_status, from_exception?"exception":"");
...
//ASUS_BSP +++ Josh_Liao "avoid bkops from exception"
//Because HPI cmd cannot be issued in this case.
#if 0
		/*
...
//ASUS_BSP +++ Josh_Liao "mmc dbg msg"
	if (err)
